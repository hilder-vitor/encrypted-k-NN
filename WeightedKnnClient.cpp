...
int decrypt_assigned_class(const vector<mpz_class>& enc_class, Paillier& paillier){
	vector<mpz_class> vec_class = paillier.dec(enc_class);
	unsigned int assigned_class = 0;
	mpz_class max_value = vec_class[0];
	for (unsigned int i = 1; i < vec_class.size(); i++){
		if (vec_class[i] > max_value){
			max_value = vec_class[i];
			assigned_class = i;
		}
	}
